...
The helper edits only the three agent `instructions` fields, nine registered skill `skill` fields, embedded Workflow Agent skill bodies, and missing registrations in the existing manifests. It preserves other asset fields and exported version markers, and fails if a standalone skill version disagrees with its embedded copy. It skips unchanged branches, creates one regular PR per changed base, and updates the deterministic sync branch when reusing an open PR.
