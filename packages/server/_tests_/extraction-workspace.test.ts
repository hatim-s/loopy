...
for (const alias of [false, true]) {
  test(`only unchanged native trace inherits recorded workspace root, canonical alias ${alias}`, async () => {
    ...
    try {
      ...
      const link = join(project, "workspace-link");
      mkdirSync(target);
      symlinkSync(target, link);
      const root = alias ? link : "/source";
      const observedRoot = alias ? realpathSync(target) : root;
      ...
      request.payload.input = { command: "bun test", cwd: `${observedRoot}/packages/service` };
      ...
      await store.commit([
        {
          ...
          run: {
            ...
            plan: {
              ...
              nodes: [],
              edges: [],
              policies: { workspace: { workingDirectory: root } },
            },
          },
        },
      ]);
      ...
      for (const changed of [false, true]) {
        ...
        if (changed) expect(verify).toBeUndefined();
          ...
          expect(review.audit).toMatchObject({
            sourceWorkspaceRoots: { [request.runId]: alias ? [root, observedRoot] : [root] },
          });
        }
      }
    } finally {
