...
export function emitJsonSchema(schema: ZodType): JsonSchema {
  return sortKeys(toJSONSchema(schema, { target: "draft-2020-12", unrepresentable: "throw", reused: "ref", cycles: "ref" })) as JsonSchema;
}
...
export function emitPublicJsonSchemas(): Record<string, JsonSchema> {
  return Object.fromEntries(Object.keys(PublicPersistedSchemas).sort().map((name) => [name, emitJsonSchema(PublicPersistedSchemas[name as keyof typeof PublicPersistedSchemas])]));
}
