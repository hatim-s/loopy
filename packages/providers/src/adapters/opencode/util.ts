...
export function eventBase(
  ...
  sessionId?: string,
): Omit<TraceEvent, "type" | "payload"> & { type: TraceEvent["type"] } {
  return {
    schemaVersion: "1",
    id: randomUUID(),
    runId: uuidFrom(context.runId ?? "opencode-run"),
    nodeId: uuidFrom(context.nodeId ?? "opencode-node"),
    attemptId: uuidFrom(context.attemptId ?? "opencode-attempt"),
    sequence: context.sequence ?? 0,
    occurredAt: context.occurredAt ?? new Date().toISOString(),
    monotonicOffsetMs: context.monotonicOffsetMs ?? 0,
    provider: "opencode",
    sessionId: sessionId ?? context.sessionId ?? "unknown-opencode-session",
    type,
...
export function parseJsonLine(line: string): {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return { error: diagnostic("malformed_event", "OpenCode JSON event must be an object.") };
    }
    return { value: parsed as Record<string, unknown> };
  } catch {
    return { error: diagnostic("malformed_event", "OpenCode emitted malformed JSON.") };
  }
}

export function isUnsafeArg(value: string): boolean {
