...
import { bootstrapSession, createApiClient, createAuthenticatedApiClient } from "./app/api";
...
async function startStudio() {
  const bootstrap = await bootstrapSession().catch((): { token?: string } => ({}));
  const api = bootstrap.token
    ? createAuthenticatedApiClient({ token: bootstrap.token })
    : createApiClient();
